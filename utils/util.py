...
def non_max_suppression(outputs, conf_threshold, iou_threshold, nc):
    ...
    for index, output in enumerate(outputs):  # image index, image inference
        ...
        if nc > 1:
            ...
            output = torch.cat((box[i], output[i, 4 + j, None], j[:, None].float(), mask[i]), 1)
        else:  # best class only
            conf, j = cls.max(1, keepdim=True)
            output = torch.cat((box, conf, j.float(), mask), 1)[conf.view(-1) > conf_threshold]

        # Check shape
        n = output.shape[0]  # number of boxes
        if not n:  # no boxes
            continue
        output = output[output[:, 4].argsort(descending=True)[:max_nms]]  # sort by confidence and remove excess boxes
